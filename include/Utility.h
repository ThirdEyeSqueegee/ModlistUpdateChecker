...
{
public:
    static void SendNotification(const std::string& version, const int compare_result) noexcept
    {
        ...
        std::jthread([=] {
            ...
            SKSE::GetTaskInterface()->AddTask([=] {
                ...
                if (compare_result <=> 0 == 0) // Save-safe
                {
                    if (!Settings::always_show_current_version)
                        RE::DebugNotification(current_version_notif.c_str());
                ...
                else if (compare_result <=> 1 == 0) // Save-breaking
                {
                    if (!Settings::always_show_current_version)
                        RE::DebugNotification(current_version_notif.c_str());
                    RE::DebugNotification(update_notif.c_str(), "UISkillsFocus");
    ...
    static int CompareVersions(const std::string& fetched, const std::string& current) noexcept
    {
        ...
        {
            const auto hotfix_cmp{ hotfix_f.value() <=> hotfix_c.value() };

            if (major_cmp == 0 && minor_cmp == 0 && (patch_cmp > 0 || hotfix_cmp > 0))
                return 0; // Save-safe
        }
        else
        {
            if (major_cmp == 0 && minor_cmp == 0 && patch_cmp > 0)
                return 0; // Save-safe
        }
